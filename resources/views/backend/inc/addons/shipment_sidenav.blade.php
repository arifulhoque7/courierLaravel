...
$shipment_count = DB::table('client_credit_amount')->where('client_id', $user_id_find)->value('shipment_count');
...
@if ($addon != null)
    ...
    @if( in_array($user_type,['customer']) && $shipment_count > 0 || in_array('1108', json_decode(Auth::user()->staff->role->permissions ?? "[]")))
    ...
    <li class="menu-item menu-item-submenu  {{ areActiveRoutes(['admin.shipments.index','admin.shipments.update','admin.shipments.import','admin.shipments.add.by.api','admin.shipments.create','admin.shipments.show'])}} @foreach(\App\Shipment::status_info() as $item) {{ areActiveRoutes([$item['route_name']])}} @endforeach " aria-haspopup="true" data-menu-toggle="hover">
        ...
        <div class="menu-submenu">
            ...
            <ul class="menu-subnav">
                ...
                @if( in_array($user_type,['customer']) || in_array('1108', json_decode(Auth::user()->staff->role->permissions ?? "[]")))
                ...
                @if( in_array($user_type,['customer']) || in_array('1107', json_decode(Auth::user()->staff->role->permissions ?? "[]")))
                ...
                @foreach(\App\Shipment::status_info() as $item)
                    @if(in_array($user_type,['customer']) || in_array($item['permissions'], json_decode(Auth::user()->staff->role->permissions ?? "[]")))
...
@if ($addon != null)
    @if($addon->activated)
        ...
        @if( in_array($user_type,['admin']) || in_array('1106', json_decode(Auth::user()->staff->role->permissions ?? "[]")))
            <li class="menu-item menu-item-submenu  {{ areActiveRoutes(['admin.transactions.index','admin.transactions.create','admin.transactions.addCredit'])}}" aria-haspopup="true" data-menu-toggle="hover">
                <a href="javascript:;" class="menu-link menu-toggle">
                ...
                <div class="menu-submenu">
                    ...
                    <ul class="menu-subnav">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.transactions.addCredit'])}}" aria-haspopup="true">
                            <a href="{{ route('admin.transactions.addCredit') }}" class="menu-link">
                                    <i class="menu-bullet menu-bullet-dot" style="font-size: 10px;"></i>
                                <span class="menu-text">{{translate('Add Credits')}}</span>
                            </a>
                            {{--  menu-item-open --}}
                        </li>
