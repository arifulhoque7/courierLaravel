                ...
                <div class="flex-wrap mr-5 d-flex align-items-baseline">
                    ...
                    <ul class="p-0 my-2 mr-5 breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold font-size-sm">
                        <li class="breadcrumb-item ">
                            <a href="{{ route('admin.dashboard')}}" class="">{{translate('Dashboard')}}</a>
                        </li>
                        <li class="breadcrumb-item ">
                            <a href="#" class="">{{$shipment->code}}</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
            ...
            <div class="col-md-10">
                ...
                <div class="pb-6 d-flex justify-content-between">
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Merchant')}}</span>
                        ...
                        <span class="font-size-md">{{$shipment->client_phone}}</span>
                        <span class="font-size-md">{{$shipment->client_address}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Customer')}}</span>
                        <span class="text-danger font-weight-boldest font-size-lg">{{$shipment->reciver_name}}</span>
                        <span class="font-size-md">{{$shipment->reciver_phone}}</span>
                        <span class="font-size-md">{{$shipment->reciver_address}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Status')}}</span>
                        <span class="d-block">{{$shipment->getStatus()}}</span>
                    </div>
                    @if ($shipment->amount_to_be_collected && $shipment->amount_to_be_collected  > 0)
                        <div class="d-flex flex-column flex-root">
                            ...
                            <span class="font-weight-bolder font-size-lg">{{format_price($shipment->amount_to_be_collected)}}</span>
                        </div>
                    @endif
                </div>
                ...
                <div class="pt-6 d-flex justify-content-between">
                    <div class="d-flex flex-column flex-root">
                        ...
                        <span class="">{{$shipment->type}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-2 font-weight-bolder">{{translate('Current branch')}}</span>
                        <a class="" href="{{route('admin.branchs.show',$shipment->branch_id)}}">{{$shipment->branch->name}}</a>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-2 font-weight-bolder">{{translate('Created date')}}</span>
                        <span class="">{{$shipment->created_at->format('Y-m-d h:i:s')}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-2 font-weight-bolder">{{translate('Shipping date')}}</span>
                        <span class="">{{$shipment->shipping_date}}</span>
                    </div>
                </div>
                ...
                <div class="pt-6 d-flex justify-content-between">
                    ...
                    @if ($shipment->prev_branch)
                        <div class="d-flex flex-column flex-root">
                            ...
                            <span class="">{{\App\Branch::find($shipment->prev_branch)->name}}</span>
                        </div>
                    @endif
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Total Weight')}}</span>
                        <span class="font-weight-bolder font-size-lg">{{$shipment->total_weight}} {{translate('KG')}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Shipping Cost')}}</span>
                        <span class="font-weight-bolder font-size-lg">{{format_price($shipment->shipping_cost)}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Tax &  Duty')}}</span>
                        <span class="font-weight-bolder font-size-lg">{{format_price($shipment->tax)}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Insurance')}}</span>
                        <span class="font-weight-bolder font-size-lg">{{format_price($shipment->insurance)}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                        <span class="mb-4 text-dark font-weight-bold">{{translate('Return Cost')}}</span>
                        <span class="font-weight-bolder font-size-lg">{{format_price($shipment->return_cost)}}</span>
                    </div>
                </div>
                ...
                <div class="pt-6 d-flex justify-content-between">
                    <div class="d-flex flex-column flex-root">
                            ...
                            <span class="font-weight-bolder font-size-lg">@if(isset($shipment->from_country->name)){{$shipment->from_country->name}} @endif </span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('To Country')}}</span>
                            <span class="font-weight-bolder font-size-lg">@if(isset($shipment->to_country->name)){{$shipment->to_country->name}} @endif </span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('From Ragion')}}</span>
                            <span class="font-weight-bolder font-size-lg">@if(isset($shipment->from_state->name)){{$shipment->from_state->name}} @endif </span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('To Ragion')}}</span>
                            <span class="font-weight-bolder font-size-lg">@if(isset($shipment->to_state->name)){{$shipment->to_state->name}} @endif </span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('From Area')}}</span>
                            <span class="font-weight-bolder font-size-lg">@if(isset($shipment->from_area->name)){{$shipment->from_area->name}} @endif </span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('To Area')}}</span>
                            <span class="font-weight-bolder font-size-lg">@if(isset($shipment->to_area->name)){{$shipment->to_area->name}} @endif </span>
                    </div>
                </div>
                ...
                <div class="pt-6 d-flex justify-content-between">
                    <div class="d-flex flex-column flex-root">
                        ...
                        <span class="font-weight-bolder font-size-lg">{{$shipment->delivery_time}}</span>
                    </div>
                    ...
                    @if ($shipment->mission_id != null)
                        <div class="d-flex flex-column flex-root">
                            ...
                            <a class="font-weight-bolder font-size-lg" href="{{route('admin.missions.show',$shipment->mission_id)}}">{{$shipment->current_mission->code}}</a>
                        </div>
                    @endif
                </div>
                @if($shipment->attachments_before_shipping)
                    <div class="pt-6 d-flex justify-content-between">
                        <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('Attachments')}} <span class=" font-size-xs">({{translate('ADDED WHEN SHIPMENT CREATED')}})</span></span>
                            <div class="pt-6 d-flex justify-content-between">
            ...
            <div class="col-md-10">
                <div class="table-responsive">
                    <table class="table">
                        <thead>
                            <tr>
                                <th class="pl-0 font-weight-bold  text-uppercase">{{translate('Package Items')}}</th>
                                <th class="text-right font-weight-bold  text-uppercase">{{translate('Qty')}}</th>
                                <th class="text-right font-weight-bold  text-uppercase">{{translate('Type')}}</th>
                                <th class="pr-0 text-right font-weight-bold  text-uppercase">{{translate('Weight x Length x Width x Height')}}</th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            <div class="col-md-10">
                <div class="table-responsive">
                    <table class="table">
                        <thead>
                            <tr>
                                <th class="font-weight-bold  text-uppercase">{{translate('PAYMENT TYPE')}}</th>
                                <th class="font-weight-bold  text-uppercase">{{translate('PAYMENT STATUS')}}</th>
                                <th class="font-weight-bold  text-uppercase">{{translate('PAYMENT DATE')}}</th>
                                <th class="text-right font-weight-bold  text-uppercase">{{translate('TOTAL COST')}}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="font-weight-bolder">
                                ...
                                <td class="text-right text-primary font-size-h3 font-weight-boldest">{{format_price($shipment->amount_to_be_collected+$shipment->tax + $shipment->shipping_cost + $shipment->insurance) }}<br /></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="pt-2 card-body" style="padding-bottom: 0;overflow:hidden">
            ...
            @foreach($shipment->logs()->orderBy('id','desc')->get() as $log)
                ...
                <div class="timeline-item align-items-start">
                    ...
                    <!--begin::Text-->
                    <div class="pl-3 font-weight-mormal font-size-lg timeline-content ">
