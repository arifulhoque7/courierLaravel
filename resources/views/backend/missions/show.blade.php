<?php
// dd($mission->client_id);
$client_phone = DB::table('clients')
    ->where('id', $mission->client_id)
    ->value('mobile');
                        ...
                        <div class="col-md-9">
                            <div class="d-flex justify-content-between pb-10 pb-md-20 flex-column flex-md-row">
                                <h1 class="display-4 font-weight-boldest mb-10">
                                    ...
                                    @else
                                        <img src="{{ static_asset('assets/img/Akash Parcel@300x.png') }}"
                                            class="d-block mb-5" style="max-width: 240px;">
                                    @endif
                                ...
                                <div class="d-flex flex-column align-items-md-end px-0">
                                    ...
                                    <span class="d-flex flex-column align-items-md-end opacity-70">
                                        <br />
                                        <span><span class="font-weight-bolder">{{ translate('CREATED DATE') }}:</span>
                                            {{ $mission->created_at->format('Y-m-d') }}</span>
                                        <span><span class="font-weight-bolder">{{ translate('MISSION CODE') }}:</span>
                                            {{ $mission->code }}</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-between pt-6">
                                ...
                                @if ($mission->type == 'transfer')
                                    <div class="d-flex flex-column flex-root">
                                        <span
                                            class="font-weight-bolder d-block mb-2">{{ translate('TRANSFER TO BRANCH') }}<span>
                                                <span class="opacity-70 d-block">{{ $mission->to_branch->name }}</span>
                                    </div>
                                @else
                                    <div class="d-flex flex-column flex-root">
                                        <span
                                            class="font-weight-bolder d-block mb-2">{{ translate('MISSION ADDRESS') }}<span>
                                                <span class="opacity-70 d-block">{{ $mission->address }}</span>
                                    </div>
                                    <div class="d-flex flex-column flex-root">
                                        <span
                                            class="font-weight-bolder d-block mb-2">{{ translate('MARCHANT PHONE NO.') }}<span>
                                                <span class="opacity-70 d-block">{{ $client_phone }}</span>
                                    </div>
                                @endif
                                <div class="d-flex flex-column flex-root">
                                    <span class="font-weight-bolder d-block mb-2">{{ translate('MISSION STATUS') }}<span>
                                            <span
                                                class="opacity-70 d-block text-{{ \App\Mission::getStatusColor($mission->status_id) }}">{{ $mission->getStatus() }}</span>
                                </div>
                                @if ($mission->captain_id)
                                    <div class="d-flex flex-column flex-root">
                                        <span
                                            class="font-weight-bolder d-block mb-2">{{ translate('MISSION Driver') }}<span>
                                                ...
                                                @if ($mission->status_id == App\Mission::REQUESTED_STATUS || $mission->status_id == App\Mission::APPROVED_STATUS || $mission->status_id == App\Mission::RECIVED_STATUS)
                                                    @if ($mission->type == \App\Mission::getType(\App\Mission::SUPPLY_TYPE))
                                                        <th class="font-weight-bold text-muted text-uppercase text-right ">
                                                            {{ translate('RETURN AMOUNT') }}</th>
                                                    @elseif($mission->type ==
                                                        \App\Mission::getType(\App\Mission::DELIVERY_TYPE))
                                                        <th class="font-weight-bold text-muted text-uppercase text-right ">
                                                            {{ translate('COD AMOUNT') }}</th>
                                                        <th class="font-weight-bold text-muted text-uppercase text-right ">
                                                            {{ translate('DELIVERY AMOUNT') }}</th>
                                                    @endif
                                                @endif
                                                <th class="font-weight-bold text-muted text-uppercase text-right ">
                        ...
                        <div class="col-md-9">
                            <div class="d-flex justify-content-between">
                                <button type="button" class="btn btn-primary font-weight-bold"
                                    onclick="window.print();">{{ translate('Print Mission') }}</button>
                            </div>
                        </div>
                    </div>
                    <!-- end: Invoice action-->
