                ...
                <tr>
                    @php
                       $shipment_id = DB::table('shipment_mission')
                        ...
                        $from = DB::table('shipments')
                                    ->where('id', $shipment_id)
                                    ->value('from_area_id');
                        $to = DB::table('shipments')
                                    ->where('id', $shipment_id)
                                    ->value('to_area_id');
                        $form_area = DB::table('areas')
                                    ->where('id', $from)
                                    ->value('name');
                        $to_area = DB::table('areas')
                                    ->where('id', $to)
                                    ->value('name');
                        //dd($form_area);
                    ...
                    <td class="text-center">
                        ...
                        @if(isset($status))
                            ...
                            @endif
                            @if($status == \App\Mission::RECIVED_STATUS|| $status == \App\Mission::REQUESTED_STATUS)
                            {{-- @php
                                dd();
                            @endphp --}}
                                ...
                                {{-- @if(Auth::user()->user_type == 'admin' || in_array(1030, json_decode(Auth::user()->staff->role->permissions ?? "[]"))) --}}
                                 @if (($mission->type == "Supply" || $mission->type == "Delivery" || $mission->type == "On-Hub") && $mission->getStatus()== "Recived" || ($mission->type == "Supply" ) && $mission->getStatus() == "Requested") 
                                <a class="btn btn-success btn-sm" data-url="{{route('admin.missions.action.confirm_amount',['mission_id'=>$mission->id])}}" data-action="POST" onclick="openAjexedModel(this,event)" href="{{route('admin.missions.show', $mission->id)}}" title="{{ translate('Show') }}">
