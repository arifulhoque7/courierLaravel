                            ...
                            <div class="d-flex justify-content-between pb-10 pb-md-20 flex-column flex-md-row">
                                ...
                                <div class="d-flex flex-column align-items-md-end px-0">
                                    <span class="d-flex flex-column align-items-md-end opacity-70">
                                        <br />
                                        <span><span class="font-weight-bolder">{{ translate('MANIFEST DATE') }}:
                                                @if (isset($due_date)) {{ $due_date }} @else {{ now()->format('Y-m-d') }} @endif</span> </span>
                                        <span><span class="font-weight-bolder">{{ translate('FOR Driver') }}:</span>
                                            {{ $captain->name }}</span>
                                    </span>
                                </div>
                            </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody id="profile_manifest">
                                        ...
                                        @foreach ($missions as $key => $mission)
                                            ...
                                            @php
                                                $shipments_user_id = \App\UserClient::where('client_id', $mission->client_id)->first();
                                                $shipments_user = \App\User::where('id', $shipments_user_id->user_id)->first();
                                                // dd($mission);
                                                // dd($shipments_user->phone);
                                                $shipment_id = DB::table('shipment_mission')
                                            ...
                                            <tr data-missionid="{{ $mission->id }}" class="mission"
                                                style="background-color:tomatom">
                                                <td></td>
                                                <td width="5%">{{ $mission->code }}</td>
                                                <td width="5%">{{ $shipment_code }}</td>
                                                <td>{{ $mission->type }}</td>
                                                @php
                                                ...
                                                <td>{{ $shipments_user->name }}</td>
                                                <td>{{ $shipments_user->phone }}</td>
                                                <td>
                                                    <div
                                                        style="width: 30px;height: 30px;border: 1px solid;border-radius: 3px;">
                                                    </div>
                                                </td>
                        ...
                        <div class="col-md-9">
                            <div class="d-flex justify-content-between">
                                <button type="button" class="btn btn-primary font-weight-bold"
                                    onclick="window.print();">{{ translate('Print Manifest') }}</button>
                            </div>
                        </div>
                    </div>
                    <!-- end: Invoice action-->
