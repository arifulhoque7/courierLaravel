					...
					<div class="login-form text-center p-7 position-relative overflow-hidden">
						...
						<div class="d-flex flex-center mb-5">
							<a href="{{ route('home') }}">
                                ...
                                @else
                                    <img src="{{ static_asset('assets/img/Akash Parcel@300x.png') }}" alt="{{ get_setting('site_name') }}" class="max-h-60px">
                                @endif
							</a>
						</div>
						<!--end::Login Header-->
						...
						<div class="login-signin">
							<div class="mb-20">
								<h3>{{ translate('Welcome to') }} @if(get_setting('site_name')) {{ get_setting('site_name') }} @else {{ translate('Akash Parcel') }}  @endif</h3>
                            ...
                            <form class="form" method="POST" role="form" action="{{ route('login') }}">
								...
								<div class="form-group mb-5">
                                    <input id="email" type="text" class="form-control h-auto form-control-solid py-4 px-8 {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus placeholder="{{ translate('Email') }}">
                                    @if ($errors->has('email'))
